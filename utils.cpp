...
namespace matrix_utils {
    ...
    void PrintResults(std::vector<double>& serial_times, std::vector<double>& parallel_omp_times, std::vector<double>& parallel_threads_times) {
        ...
        std::cout << "---------------------------------------------------------------------------------------------------------------\n";
        std::cout << std::fixed;
        std::cout << std::setw(1) << "-  " << std::setw(6) << "Average" << std::setw(4) << "|" << std::setw(13) << std::setprecision(6) << avg_serial << std::setw(13) << "|" << std::setw(16) << std::setprecision(6) << avg_parallel_omp << std::setw(16) << "|" << std::setw(20) << std::setprecision(6) << avg_parallel_threads << "\n";

        double performance_increase_parallel_omp {};
        double performance_increase_parallel_threads {};

        if(avg_parallel_omp < avg_serial) performance_increase_parallel_omp = ((avg_serial - avg_parallel_omp) / avg_serial) * 100.0;
        if(avg_parallel_threads < avg_serial) performance_increase_parallel_threads = ((avg_serial - avg_parallel_threads) / avg_serial) * 100.0;

        std::cout << "---------------------------------------------------------------------------------------------------------------\n";
        std::cout << std::fixed;
        std::cout << std::setw(1) << " " << std::setw(6) << "% vs Serial" << std::setw(2) << "|" << std::setw(12) << "-" << std::setw(14) << "|" << std::setw(15) << std::setprecision(2) << performance_increase_parallel_omp << "%" << std::setw(16) << "|" << std::setw(19) << std::setprecision(2) << performance_increase_parallel_threads << "%" << "\n";

        std::cout << std::fixed;
        std::cout << "\n<> Best performing algorithm is: ";
        if(avg_parallel_omp < avg_serial && avg_parallel_omp < avg_parallel_threads) {
            std::cout << "Parallel OMP Algorithm with an average of " << std::setprecision(6) << avg_parallel_omp << " (ms).\n\n";
        } else if(avg_parallel_threads < avg_serial && avg_parallel_threads < avg_parallel_omp) {
            std::cout << "Parallel POSIX Threads Algorithm with an average of " << std::setprecision(6) << avg_parallel_threads << " (ms).\n\n";
        } else {
            std::cout << "Serial Algorithm with an average of " << std::setprecision(6) << avg_serial << " (ms).\n\n";
        }
    }
